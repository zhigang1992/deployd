...
Dashboard.prototype.handle = function(ctx, next) {
  ...
  } else if (ctx.url.indexOf('/module-middleware') === 0) {
    this.renderModuleMiddleware(ctx);
...
Dashboard.prototype.renderModuleMiddleware = function(ctx) {
  var self = this;

  var deploymentsPageQ = q.ninvoke(fs, 'readFile', path.join(__dirname, 'dashboard/middleware.html'), 'utf-8');
